...
export default function Home() {
  ...
  const [ditherAlgorithm, setDitherAlgorithm] = useState<DitherAlgorithm>("bayer4");
  ...
  const resetImageSettings = useCallback(() => {
    ...
    setColorMode("colour");
    setColorCount(0);
    setDitherAlgorithm("bayer4");
    setRenderMode("hybrid");
    setAdjustments(DEFAULT_IMAGE_ADJUSTMENTS);
    setStatus("Image settings reset to Balanced.");
  }, []);
  ...
  useEffect(() => {
    const hydrate = () => {
      ...
      const numeric = (key: string, fallback: number) => { const value = Number(params.get(key)); return Number.isFinite(value) ? value : fallback; };
      ...
      <div className="grid flex-1 gap-4 xl:grid-cols-[292px_minmax(0,1fr)_292px]">
        <section className="space-y-3 rounded-md border border-white/10 bg-white/[0.03] p-3 backdrop-blur-sm">
          ...
          {mode === "image" ? <>
            ...
            <div className="space-y-2 rounded-md border border-white/10 bg-black/40 p-3"><h2 className="text-[10px] uppercase tracking-[0.3em] text-slate-500">Character set</h2><div className="grid gap-1.5">{CHARACTER_SET_OPTIONS.map((option) => <button key={option.id} type="button" onClick={() => setCharacterSet(option.id)} className={`rounded-sm border px-3 py-2 text-left transition hover:bg-white/10 ${characterSet === option.id ? "border-emerald-300/40 bg-emerald-300/10" : "border-white/10 bg-white/[0.03]"}`}><span className="text-sm text-white">{option.label}</span><span className="mt-1 block font-mono text-[10px] text-slate-500">{option.sample}</span></button>)}</div>{characterSet === "custom" ? <textarea value={customGlyphs} onChange={(event) => setCustomGlyphs(event.target.value)} rows={2} className="w-full rounded-sm border border-white/10 bg-black/70 px-3 py-2 font-mono text-sm text-white outline-none focus:border-emerald-300/40" placeholder="@#%" /> : null}<p className="text-xs text-slate-500">Generic and custom sets are ordered by measured glyph density.</p></div>
